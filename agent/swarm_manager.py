...
class SwarmRunConfig:
    ...
    # Alias: some callers refer to swarm iterations as "rounds".
    # If provided, and max_cycles is None, we treat max_rounds as max_cycles.
    ...
    def __post_init__(self) -> None:
        # Backward/forward compatibility: if a caller specifies max_rounds,
        # use it as max_cycles unless max_cycles was explicitly set.
        if self.max_cycles is None and self.max_rounds is not None:
            try:
                self.max_cycles = int(self.max_rounds)
            except Exception:
                pass
...
class SwarmManager:
    ...
    def attach_agents(self, agents: List[CoreAgent]) -> None:
        ...
        max_safe = int(self._swarm_global_hints.get("max_agents_safe", 32))
