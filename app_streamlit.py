...
def _load_jsonl_events_file(path: Path, max_lines: int = 2000) -> Optional[List[Dict[str, Any]]]:
    """Load events from a JSONL file.

    Returns:
      - None if the file does not exist / is unreadable
      - [] if the file exists but no parseable JSON objects were found
      - A list[dict] for successfully parsed event objects
    """
    try:
        if not isinstance(path, Path):
            path = Path(path)
        ...
        # Only parse the tail to keep the UI responsive even if logs grow large.
        ...
        for line in lines:
            ...
            if isinstance(obj, dict):
                ...
                # Normalize message field across schemas (event_log.py vs events.py vs CoreAgent JSONL).
                if "message" not in ev:
                    if "msg" in ev:
                        ev["message"] = ev.get("msg")
                    elif "text" in ev:
                        ev["message"] = ev.get("text")
                    elif "summary" in ev:
                        ev["message"] = ev.get("summary")

                # Normalize kind/type
                if "kind" not in ev:
                    ev["kind"] = ev.get("type") or ev.get("domain") or "event"

                # Some emitters use "timestamp" instead of numeric "ts"
                if "ts" not in ev and "timestamp" in ev:
                    ev["ts"] = ev.get("timestamp")

                events.append(ev)

            elif isinstance(obj, list):
                # Occasionally emitters write a list on a line; accept dict items.
                for item in obj:
                    if isinstance(item, dict):
...
def _first_existing_jsonl(paths: List[Path], max_lines: int = 2000) -> Tuple[Optional[List[Dict[str, Any]]], Optional[Path]]:
    """Return (events, path) for the best available JSONL event log.

    Preference order:
    1) First file that yields at least one parseable event line.
    2) Otherwise, the first existing JSONL file (even if empty/unparseable) so the UI can
       stop claiming it "doesn't exist".
    """
    first_existing: Optional[Tuple[List[Dict[str, Any]], Path]] = None

    for p in paths:
        evs = _load_jsonl_events_file(p, max_lines=max_lines)
        if evs is None:
            continue

        if first_existing is None:
            first_existing = (evs, p)

        if evs:
            return evs, p

    if first_existing is not None:
        return first_existing[0], first_existing[1]
...
def load_event_log_unified(run_id: Optional[str]) -> Tuple[List[Dict[str, Any]], str]:
    ...
    Supports both:
      - JSON files (event_log.json, <run_id>_event_log.json, etc.)
      - JSONL files (events.jsonl, event_log.jsonl, etc.)

    If no event log exists yet, this returns an empty list and the caller can synthesize from history.
    """
    # 1) Try legacy JSON logs first (expected by the raw JSON expander).
    ...
    if isinstance(raw, dict):
        ...
        if isinstance(maybe, list):
            ...
            if run_id and any("run_id" in e for e in events):
                rid = str(run_id)
                events = [e for e in events if str(e.get("run_id")) == rid]
    ...
    # 2) Fall back to JSONL logs (events.py / CoreAgent JSONL emitters).
    runs_root = Path(get_runs_root())
    ...
    if run_id:
        ...
        # Per-run JSONL (most reliable when present)
        jsonl_paths.extend(
            [
                run_dir / "events.jsonl",
                run_dir / "event_log.jsonl",
                run_dir / "logs" / "events.jsonl",
                run_dir / "logs" / "event_log.jsonl",
                logs_dir / f"{rid}_event_log.jsonl",
                logs_dir / f"{rid}_events.jsonl",
            ]
        )

    # Shared logs JSONL (common fallbacks)
    jsonl_paths.extend(
        [
            logs_dir / "event_log.jsonl",
            logs_dir / "events.jsonl",
            logs_dir / "events_global.jsonl",
            logs_dir / "event_log_global.jsonl",
            queue_root / "event_log.jsonl",
            queue_root / "events.jsonl",
        ]
    )

    evs, p2 = _first_existing_jsonl(jsonl_paths, max_lines=2000)
    if p2 is not None and evs is not None:
        if run_id and any("run_id" in e for e in evs):
            rid = str(run_id)
            evs = [e for e in evs if str(e.get("run_id")) == rid]
        return evs, str(p2)
        ...
        with st.expander("Raw event log JSON (if available)"):
            if event_log0:
                ...
                if preview:
                    ...
                    if note:
                        st.caption(note)
            elif event_src0 != "not found":
                st.info("Event log file found, but no events were parsed yet.")
                st.caption(f"Source: `{event_src0}`")
            else:
